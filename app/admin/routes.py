...
def dashboard():
    ...
    # 2. Toplam Müşteri Sayısını Hesapla (HATA BURADAYDI, ARTIK TANIMLI)
    cust_resp = supabase.table('customers').select('id', count='exact').execute()
    # Supabase count dönerse onu al, dönmezse listenin uzunluğunu al
    total_customers = cust_resp.count if cust_resp.count is not None else len(cust_resp.data)
    ...
    # 3. Tüm değişkenleri HTML'e gönder
...
def add_customer():
    ...
    try:
        ...
        flash(f'Müşteri eklenirken hata oluştu.', 'danger')
...
def update_customer(customer_id):
    ...
    update_data["status"] = "Askıda" if end_date_obj < today else "Aktif"

    try:
        supabase.table('customers').update(update_data).eq('id', customer_id).execute()
        flash(f'{full_name} bilgileri güncellendi.', 'success')
    except Exception as e:
